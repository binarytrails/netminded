...
function draw() {
	...
	checkType();
	if (currentType < 5) {
	image(typeArray[currentType][currentMovie].img, 20, 20, girlyArray[2].img.width/4, girlyArray[2].img.height/4);
	text(typeArray[currentType][currentMovie].title, width/2, 20);
	} else
	text("You look heavenly boring, time to have personality bro'", width/2, 20);
}

function checkType() {
	if (myColor.pinkColor)
	currentType = 0;
	if (myColor.redColor)
	currentType = 1;
	if (myColor.blueColor || myColor.greenColor)
	currentType = 2;
	if (myColor.yellowColor)
	currentType = 3;
	if (myColor.blackColor)
	currentType = 4;
	if (myColor.whiteColor)
	currentType = 5;
}
...
function keyPressed() {
  	if (keyCode == UP_ARROW){
  		currentMovie =  int(random(29));
  		console.log(currentMovie);
  	}
  	else {
  	myColor.setColor(keyCode);
  	}	
  }
...
function colorObject() {
  	...
  	//this.main = whiteColor;
  	this.pinkColor = false;
	this.redColor = false;
	...
	this.setColor = function(myKeyCode) {
		//resets all the values
  		...
  		this.redColor = true;
  		//if b is pressed
  		else if (myKeyCode == 66)
  		this.blueColor = true;
  		//if g is pressed
  		else if (myKeyCode == 71)
